...
# run it
#
echo "[wrapper] running: python skim_bennettworkflow/skim.py skim_bennettworkflow/selection_${SKIMID}.txt ${FILE} ${FILEID}.root"

python skim_bennettworkflow/skim.py skim_bennettworkflow/selection_${SKIMID}.txt ${FILE} ${FILEID}.root 
...
if not f1 or not f1.IsOpen() or f1.IsZombie():
    print "[RSR] removing zombie ${OUTPUT} because it does not deserve to live"
    os.system("rm ${OUTPUT}")
    sys.exit()
...
if foundBad:
    print "[RSR] removing ${OUTPUT} because it does not deserve to live"
    os.system("rm ${OUTPUT}")
else:
