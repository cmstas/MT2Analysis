...
void MT2Looper::SetSignalRegions(){
  ...
  //SRVec = getSignalReglions2015LowLumi(); //Phys14 AN selection
  ...
  //SRVec =  getSignalRegionsSoftMuon(); //same as getSignalRegionsZurich_jetpt30(), but with with small modifications (MT2 > 100)
  ...
  while ( (currentFile = (TFile*)fileIter.Next()) ) {
	...
	if (doNvtxReweight) {
      ...
      for (int i=0; i < t.ngenLep; i++){
        plot1D("hg_allgenLep_pt",       t.genLep_pt[i],      evtweight_, h_1d_global, ";p_{T}(genLep)[GeV]", 200, 0, 1000);
        if (t.genLep_pt[i] < 20 && t.genLep_pt[i] > 5 )
          plot1D("hg_genLep_pt",       t.genLep_pt[i],    evtweight_, h_1d_global, ";p_{T}(genLep)[GeV]", 50, 0, 25);
      }
      for (int i=0; i < t.ngenLepFromTau; i++){
        plot1D("hg_allgenLepfTau_pt",   t.genLepFromTau_pt[i],       evtweight_, h_1d_global, ";p_{T}(genLepFromTau)[GeV]", 200, 0, 1000);
        if (t.genLepFromTau_pt[i] < 20 && t.genLepFromTau_pt[i] > 5 )
          plot1D("hg_genLepfTau_pt",   t.genLepFromTau_pt[i],       evtweight_, h_1d_global, ";p_{T}(genLepFromTau)[GeV]", 50, 0, 25);
      }
...
/*
      ...
      if (t.nMuons10 == 1){
        for (int ilep = 0; ilep < t.nlep; ++ilep) {
          ...
          smureliso_ = t.lep_relIso03[ilep];
          smuabsiso_ = t.lep_relIso03[ilep] * smupt_;
          smumreliso_ = t.lep_miniRelIso[ilep];
          smusip_ = -1;
...
*/
      ...
      if (nIsoMuons == 1 && t.softmus_pt[iism] < 20) { 
        ...
        smuabsiso_  = t.softmus_absIso[iism];
        smureliso_  = t.softmus_relIso03[iism];
        smumreliso_ = t.softmus_miniRelIso[iism];
        smusip_     = t.softmus_sip[iism];
        mt_ = sqrt( 2 * t.met_pt * smupt_ * ( 1 - cos( t.met_phi - smuphi_) ) );
      ...
      //   //nGenButNotReco++;
      //   cout << "------------------------------------\n" 
      //        << "t.nsoftmus = " << t.nsoftmus << endl;
      //   for(int i=0; i < t.nsoftmus; i++){
      //     cout << "t.softmus_isReco[" << i << "] = " << t.softmus_isReco[i] << endl;
      //     cout << "t.softmus_isPF[" << i << "] = " << t.softmus_isPF[i] << endl;
      //     cout << "t.softmus_pt[" << i << "] = " << t.softmus_pt[i] << endl;
      //     cout << "t.softmus_eta[" << i << "] = " << t.softmus_eta[i] << endl;
      //     cout << "t.softmus_phi[" << i << "] = " << t.softmus_phi[i] << endl;
      //     cout << "t.softmus_absIso[" << i << "] = " << t.softmus_absIso[i] << endl;
      //     cout << "t.softmus_miniRelIso[" << i << "] = " << t.softmus_miniRelIso[i] << endl;
      //     cout << "t.softmus_sip[" << i << "] = " << t.softmus_sip[i] << endl;
      //     cout << "------------------------------------\n";
      //   }
        
      //   if(t.ngenLep ==1 ){
      //     cout << "t.genLep_pt[0] = " << t.genLep_pt[0] << endl;
      //     cout << "t.genLep_eta[0] = " << t.genLep_eta[0] << endl;
      //     cout << "t.genLep_phi[0] = " << t.genLep_phi[0] << endl;
      //   }
      //   else if(t.ngenLepFromTau == 1){
      //     cout << "t.genLepFromTau_pt[0] = " << t.genLepFromTau_pt[0] << endl;
      //     cout << "t.genLepFromTau_eta[0] = " << t.genLepFromTau_eta[0] << endl;
      //     cout << "t.genLepFromTau_phi[0] = " << t.genLepFromTau_phi[0] << endl;
      //   }
      //   cout << "------------------------------------\n";
      //   cout << endl;
      // }
...
//		) 
      ...
      // Variables for Soft lepton regions
      //muon only
      bool doSoftLplots = false;
	...
	for ( int k = 0; k < t.ngenPart; k++) {
	  if (abs(t.genPart_pdgId[k])==24 && t.genPart_status[k] == 22) Widx_=k;
	  if (abs(t.genPart_pdgId[k])==14 && t.genPart_status[k] == 23) NUidx_=k;
	}

	if (Widx_ == -1 || NUidx_ == -1 ) continue;
	
	// if ( t.genPart_pt[Widx_] > 200 ) {
	//   cout << "---------------------" << endl;
	//   cout << "w index : " << Widx_ << endl;
	...
	//fillHistosCRGJ("crgj");
      ...
      if (doSoftLplots) {
        saveSoftLplots = true;
        fillHistosSoftL("srsoftl");
      }
      if (doSoftLCRplots) {
        saveSoftLCRplots = true;
        fillHistosSoftLCR("crsoftl");
      }
      if (doSLplots) {
        saveSLplots = true;
        fillHistosCRSL("crsl");
  ...
  if (saveSoftLplots) {
    for(unsigned int srN = 0; srN < SRVec.size(); srN++){
      if(!SRVec.at(srN).srsoftlHistMap.empty()){
        savePlotsDir(SRVec.at(srN).srsoftlHistMap, outfile_, ("srsoftl"+SRVec.at(srN).GetName()).c_str());
      }
    }
  }
  if (saveSoftLCRplots) {
    for(unsigned int srN = 0; srN < SRVec.size(); srN++){
      if(!SRVec.at(srN).crsoftlHistMap.empty()){
        savePlotsDir(SRVec.at(srN).crsoftlHistMap, outfile_, ("crsoftl"+SRVec.at(srN).GetName()).c_str());
      }
    }
  }
  if (saveSLplots) {
...
void MT2Looper::fillHistosSoftL(const std::string& prefix, const std::string& suffix) {
  ...
  // cout << "diffMetMhtOverMet: " << values["diffMetMhtOverMet"] << endl;
  // cout << "nlep: " << values["nlep"] << endl;
  // cout << "j1pt: " << values["j1pt"] << endl;
  // cout << "j2pt: " << values["j2pt"] << endl;
  // cout << "njets: " << values["njets"] << endl;
  // cout << "nbjets: " << values["nbjets"] << endl;
  // cout << "mt2: " << values["mt2"] << endl;
  // cout << "ht: " << values["ht"] << endl;
  // cout << "met: " << values["met"] << endl;
  
  for(unsigned int srN = 0; srN < SRVec.size(); srN++){
    if(SRVec.at(srN).PassesSelection(values)){
      fillHistosSoftLepton(SRVec.at(srN).srsoftlHistMap, SRVec.at(srN).GetNumberOfMT2Bins(), SRVec.at(srN).GetMT2Bins(), prefix+SRVec.at(srN).GetName(), suffix);
      break;//signal regions are orthogonal, event cannot be in more than one
    }
  }
...
void MT2Looper::fillHistosSingleSoftMuon(std::map<std::string, TH1*>& h_1d, int n_mt2bins, float* mt2bins, const std::string& dirname, const std::string& s) {
  ...
  if(smupt_ < 10 )      plot1D("h_mt_mpt5-10"+s,   mt_,   evtweight_, h_1d, ";M_{T} [GeV]", 200, 0, 500);
  else if(smupt_ < 15 ) plot1D("h_mt_mpt10-15"+s,  mt_,   evtweight_, h_1d, ";M_{T} [GeV]", 200, 0, 500);
  else                  plot1D("h_mt_mpt15-20"+s,  mt_,   evtweight_, h_1d, ";M_{T} [GeV]", 200, 0, 500);
  ...
  plot1D("h_smu_reliso"+s,   smureliso_,   evtweight_, h_1d, ";relIso(#mu) [GeV]", 200, 0, 50);
  ...
  if (fromGenTau) {
    ...
    plot1D("h_matMupt"+s,     smupt_,   evtweight_, h_1d, ";p_{T}(reco with gen match)", 50, 0, 25);
  }
  else{
    ...
    if (fromGenLep) {
      ...
      plot1D("h_smuMotherId"+s,      0,   evtweight_, h_1d, ";Mother pdgId of #mu", 30, 0, 30);
    }
  }
...
void MT2Looper::fillHistosSoftLCR(const std::string& prefix, const std::string& suffix) {
  ...
  values["diffMetMhtOverMet"]  = t.diffMetMht/t.met_pt; //use reco MET for this cut
  values["nlep"]        = 0; // dummy value
  values["j1pt"]        = t.jet1_pt;
  values["j2pt"]        = t.jet2_pt;
  values["njets"]       = t.nJet30;
  values["nbjets"]      = t.nBJet20;
  values["mt2"]         = t.softlep_mt2;
  values["ht"]          = t.softlep_ht;
  values["met"]         = t.softlep_met_pt;
  ...
  for(unsigned int srN = 0; srN < SRVec.size(); srN++){
    if(SRVec.at(srN).PassesSelection(values)){
      fillHistosSoftLepton(SRVec.at(srN).crsoftlHistMap, SRVec.at(srN).GetNumberOfMT2Bins(), SRVec.at(srN).GetMT2Bins(), prefix+SRVec.at(srN).GetName(), suffix);
      break;//signal regions are orthogonal, event cannot be in more than one
    }
  }
...
void MT2Looper::fillHistos(std::map<std::string, TH1*>& h_1d, int n_mt2bins, float* mt2bins, const std::string& dirname, const std::string& s) {
  ...
  // For sanity check:
  for (int i=0; i < t.ngenLep; i++){
    plot1D("h_allgenLep_pt",       t.genLep_pt[i],      evtweight_, h_1d, ";p_{T}(genLep)[GeV]", 200, 0, 1000);
    if (t.genLep_pt[i] < 20 && t.genLep_pt[i] > 5 )
      plot1D("h_genLep_pt",       t.genLep_pt[i],    evtweight_, h_1d, ";p_{T}(genLep)[GeV]", 50, 0, 25);
  }
  for (int i=0; i < t.ngenLepFromTau; i++){
    plot1D("h_allgenLepfTau_pt",   t.genLepFromTau_pt[i],       evtweight_, h_1d, ";p_{T}(genLepFromTau)[GeV]", 200, 0, 1000);
    if (t.genLepFromTau_pt[i] < 20 && t.genLepFromTau_pt[i] > 5 )
      plot1D("h_genLepfTau_pt",   t.genLepFromTau_pt[i],       evtweight_, h_1d, ";p_{T}(genLepFromTau)[GeV]", 50, 0, 25);
  }
...
void MT2Looper::fillHistosSoftLepton(std::map<std::string, TH1*>& h_1d, int n_mt2bins, float* mt2bins, const std::string& dirname, const std::string& s) {
  TDirectory * dir = (TDirectory*)outfile_->Get(dirname.c_str());
  if (dir == 0) {
    dir = outfile_->mkdir(dirname.c_str());
  } 
  ...
  if ( t.lep_pt[0] < 20 ) {
    plot1D("h_thismet"+s,       t.met_pt,   evtweight_, h_1d, ";E_{T}^{miss} [GeV]", 80, 200, 1000);
  ...
  //W plots
  plot1D("h_Wpt"+s,   t.genPart_pt[Widx_],   evtweight_, h_1d, ";p_{T} [GeV]", 250, 0, 1000);
  plot1D("h_Weta"+s,  abs(t.genPart_eta[Widx_]),   evtweight_, h_1d, ";#eta", 50, 0, 5);
  plot1D("h_Wphi"+s,  t.genPart_phi[Widx_],   evtweight_, h_1d, ";#phi_{W}", 64, -3.2, 3.2);
