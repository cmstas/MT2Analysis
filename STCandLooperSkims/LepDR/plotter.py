...
for sample in samples:
    ...
    h_el_dr = utils.GetUnderOverHist(f,"h_el_dr",over=False) #.Clone(sample+"eldr")
    h_mu_dr = utils.GetUnderOverHist(f,"h_mu_dr",color=ROOT.kRed,over=False) #.Clone(sample+"mudr")
    h_tau_dr = utils.GetUnderOverHist(f,"h_tau_dr",color=ROOT.kGreen,over=False) #.Clone(sample+"taudr")
...
#    tl.Draw()
    ...
    h_sh_el_dr.Rebin(5)
    h_sh_mu_dr = utils.GetUnderOverHist(f,"h_sh_mu_dr",color=ROOT.kRed,over=False) #.Clone(sample+"mudr")
    h_sh_mu_dr.Rebin(5)
    h_sh_tau_dr = utils.GetUnderOverHist(f,"h_sh_tau_dr",color=ROOT.kGreen,over=False) #.Clone(sample+"taudr")
    h_sh_tau_dr.Rebin(5)
