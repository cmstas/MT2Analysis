...
for sample in samples[0:4]:
    h_DR1 = (utils.GetUnderOverHist(filedict[sample],"h_lep1DR",color=sample_colors[sample],under=False)).Clone("h_DR1"+sample)
    if (h_DR1.Integral() > 0): h_DR1.Scale(1/h_DR1.Integral())
    h_DR1.SetLineWidth(3)
    DR1_histlist.append(h_DR1)
    h_DR2 = (utils.GetUnderOverHist(filedict[sample],"h_lep2DR",color=sample_colors[sample],under=False)).Clone("h_DR2"+sample)
    if (h_DR2.Integral() > 0): h_DR2.Scale(1/h_DR2.Integral())
    h_DR2.SetLineWidth(3)
    DR2_histlist.append(h_DR2)
DR1_histlist[0].SetMaximum(1.2 * max([h.GetMaximum() for h in DR1_histlist]))
DR2_histlist[0].SetMaximum(1.2 * max([h.GetMaximum() for h in DR2_histlist]))
DR1_histlist[0].SetMinimum(0)
DR2_histlist[0].SetMinimum(0)
DR1_histlist[0].Draw()
for h in DR1_histlist[1:]:
    h.Draw("same")
pads[1].cd()
tl.Draw()
canvas.SaveAs("{0}/h_DR1.png".format(outdir))
pads[0].cd()
DR2_histlist[0].Draw()
for h in DR2_histlist[1:]:
    h.Draw("same")
pads[1].cd()
tl.Draw()
canvas.SaveAs("{0}/h_DR2.png".format(outdir))
pads[0].cd()
...
for sample in samples[0:4]:
    ...
    if (h_LT.Integral() > 0): h_LT.Scale(1/h_LT.Integral())
    h_LT.SetLineWidth(3)
...
# Now, Pre-Selection LT

LT_histlist = []
for sample in samples[0:4]:
    h_LT = (utils.GetUnderOverHist(filedict[sample],"h_lepTypePS",color=sample_colors[sample])).Clone("h_lepTypePS"+sample)
    if (h_LT.Integral() > 0): h_LT.Scale(1/h_LT.Integral())
    h_LT.SetLineWidth(3)
    LT_histlist.append(h_LT)
...
LT_histlist[0].SetMaximum( 0.6 )
LT_histlist[0].SetMinimum(0)
LT_histlist[0].SetTitle("Prompt Leptons Inclusive in N_{ST}")
LTbinlabels = ["No leps","e","#mu","#tau_{e}","#tau_{#mu}","#tau_{1}","#tau_{3}",
               "ee","e#mu","e#tau_{e}","e#tau_{#mu}","e#tau_{1}","e#tau_{3}",
               "#mu#mu","#mu#tau_{e}","#mu#tau_{#mu}","#mu#tau_{1}","#mu#tau_{3}",
               "#tau_{e}#tau_{e}","#tau_{e}#tau_{#mu}","#tau_{e}#tau_{1}","#tau_{e}#tau_{3}",
...
pads[1].cd()
tl.Draw()
canvas.SaveAs("{0}/h_LTps.png".format(outdir))
pads[0].cd()
...
LT_histlist[0].SetMaximum( 1.2 * max([ h.GetMaximum() for h in LT_histlist] ) )
LT_histlist[0].SetMinimum( -1 * LT_histlist[0].GetMaximum() )
LT_histlist[0].SetTitle("Prompt Leptons Inclusive in N_{ST}, Relative Enhancement")
LT_histlist[0].Draw()
for h in LT_histlist[1:]:
    h.Draw("same")
pads[1].cd()
tl.Draw()
canvas.SaveAs("{0}/h_LTps_rel.png".format(outdir))
pads[0].cd()

factor_hists = ROOT.TFile.Open("{0}/factor_hists.root".format(outdir),"RECREATE")
factor_hists.cd()
...
h_factor_stc1 = ROOT.TH1F("h_factor_stc1L","Reduction Factor: N_{STC} > 0, L H_{T}",len(samples),0,len(samples));
h_factor_stc2 = ROOT.TH1F("h_factor_stc2L","Reduction Factor: N_{STC} > 1, L H_{T}",len(samples),0,len(samples));
h_factor_st1 = ROOT.TH1F("h_factor_st1L","Reduction Factor: N_{ST} > 0 when N_{STC} > 0 L H_{T}",len(samples),0,len(samples));
h_factor_st2 = ROOT.TH1F("h_factor_st2L","Reduction Factor: N_{ST} > 0 when N_{STC} > 1 L H_{T}",len(samples),0,len(samples));
for index,sample in enumerate(samples):
    bin = index + 1
    incl_count = incl_counts[sample]
    h_st = (utils.GetUnderOverHist(filedict[sample],"h_stL",color=sample_colors[sample])).Clone("h_stL_tmp")
    h_stc = (utils.GetUnderOverHist(filedict[sample],"h_stcL",color=sample_colors[sample])).Clone("h_stcL_tmp")
    n_st_gt0 = h_st.Integral(2,-1)
    n_stc_gt0 = h_stc.Integral(2,-1)
    n_stc_gt1 = h_stc.Integral(3,-1)
...
h_factor_stc1 = ROOT.TH1F("h_factor_stc1M","Reduction Factor: N_{STC} > 0, M H_{T}",len(samples),0,len(samples));
h_factor_stc2 = ROOT.TH1F("h_factor_stc2M","Reduction Factor: N_{STC} > 1, M H_{T}",len(samples),0,len(samples));
h_factor_st1 = ROOT.TH1F("h_factor_st1M","Reduction Factor: N_{ST} > 0 when N_{STC} > 0 M H_{T}",len(samples),0,len(samples));
h_factor_st2 = ROOT.TH1F("h_factor_st2M","Reduction Factor: N_{ST} > 0 when N_{STC} > 1 M H_{T}",len(samples),0,len(samples));
for index,sample in enumerate(samples):
    bin = index + 1
    incl_count = incl_counts[sample]
    h_st = (utils.GetUnderOverHist(filedict[sample],"h_stM",color=sample_colors[sample])).Clone("h_stM_tmp")
    h_stc = (utils.GetUnderOverHist(filedict[sample],"h_stcM",color=sample_colors[sample])).Clone("h_stcM_tmp")
    n_st_gt0 = h_st.Integral(2,-1)
    n_stc_gt0 = h_stc.Integral(2,-1)
    n_stc_gt1 = h_stc.Integral(3,-1)
...
h_factor_stc1 = ROOT.TH1F("h_factor_stc1H","Reduction Factor: N_{STC} > 0, H H_{T}",len(samples),0,len(samples));
h_factor_stc2 = ROOT.TH1F("h_factor_stc2H","Reduction Factor: N_{STC} > 1, H H_{T}",len(samples),0,len(samples));
h_factor_st1 = ROOT.TH1F("h_factor_st1H","Reduction Factor: N_{ST} > 0 when N_{STC} > 0 H H_{T}",len(samples),0,len(samples));
h_factor_st2 = ROOT.TH1F("h_factor_st2H","Reduction Factor: N_{ST} > 0 when N_{STC} > 1 H H_{T}",len(samples),0,len(samples));
for index,sample in enumerate(samples):
    bin = index + 1
    incl_count = incl_counts[sample]
    h_st = (utils.GetUnderOverHist(filedict[sample],"h_stH",color=sample_colors[sample])).Clone("h_stH_tmp")
    h_stc = (utils.GetUnderOverHist(filedict[sample],"h_stcH",color=sample_colors[sample])).Clone("h_stcH_tmp")
    n_st_gt0 = h_st.Integral(2,-1)
    n_stc_gt0 = h_stc.Integral(2,-1)
    n_stc_gt1 = h_stc.Integral(3,-1)
...
h_factor_stc1 = ROOT.TH1F("h_factor_stc1","Reduction Factor: N_{STC} > 0,",len(samples),0,len(samples));
h_factor_stc2 = ROOT.TH1F("h_factor_stc2","Reduction Factor: N_{STC} > 1,",len(samples),0,len(samples));
h_factor_st1 = ROOT.TH1F("h_factor_st1p","Reduction Factor: N_{ST} > 0 when N_{STC} > 0 (p)",len(samples),0,len(samples));
h_factor_st2 = ROOT.TH1F("h_factor_st2p","Reduction Factor: N_{ST} > 0 when N_{STC} > 1 (p)",len(samples),0,len(samples));
for index,sample in enumerate(samples):
    bin = index + 1
    incl_count = incl_counts[sample]
    h_st = (utils.GetUnderOverHist(filedict[sample],"h_stp",color=sample_colors[sample])).Clone("h_stp_tmp")
    h_stc = (utils.GetUnderOverHist(filedict[sample],"h_stc",color=sample_colors[sample])).Clone("h_stc_tmp")
    n_st_gt0 = h_st.Integral(2,-1)
    n_stc_gt0 = h_stc.Integral(2,-1)
    n_stc_gt1 = h_stc.Integral(3,-1)
...
h_factor_st1 = ROOT.TH1F("h_factor_st1s","Reduction Factor: N_{ST} > 0 when N_{STC} > 0 (s)",len(samples),0,len(samples));
h_factor_st2 = ROOT.TH1F("h_factor_st2s","Reduction Factor: N_{ST} > 0 when N_{STC} > 1 (s)",len(samples),0,len(samples));
for index,sample in enumerate(samples):
    bin = index + 1
    incl_count = incl_counts[sample]
    h_st = (utils.GetUnderOverHist(filedict[sample],"h_sts",color=sample_colors[sample])).Clone("h_sts_tmp")
    h_stc = (utils.GetUnderOverHist(filedict[sample],"h_stc",color=sample_colors[sample])).Clone("h_stc_tmp")
    n_st_gt0 = h_st.Integral(2,-1)
    n_stc_gt0 = h_stc.Integral(2,-1)
    n_stc_gt1 = h_stc.Integral(3,-1)
...
h_factor_st1 = ROOT.TH1F("h_factor_st1l","Reduction Factor: N_{ST} > 0 when N_{STC} > 0 (l)",len(samples),0,len(samples));
h_factor_st2 = ROOT.TH1F("h_factor_st2l","Reduction Factor: N_{ST} > 0 when N_{STC} > 1 (l)",len(samples),0,len(samples));
for index,sample in enumerate(samples):
    bin = index + 1
    incl_count = incl_counts[sample]
    h_st = (utils.GetUnderOverHist(filedict[sample],"h_stl",color=sample_colors[sample])).Clone("h_stl_tmp")
