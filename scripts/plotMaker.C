...
TCanvas* makePlot( const vector<TFile*>& samples , const vector<string>& names , const string& histdir , const string& histname , const string& xtitle , const string& ytitle , float xmin , float xmax , int rebin = 1 , bool logplot = true, bool printplot = false, float scalesig = -1., bool doRatio = false ) {
  ...
  if (doRatio) {
    ...
    if (!foundData) {
      ofile << "ratio requested but no data hist found.  Not plotting ratio" << endl;
      doRatio = false;
    }
  } // if doRatio
  ...
  TLegend* leg = new TLegend(0.65,0.7,0.85,0.90);
  leg->SetFillColor(0);
  leg->SetBorderSize(0);
  ...
  // soft muon
  if ((histdir.find("srsm") != std::string::npos)) region_label += ", 1 muon";
  // soft electron when no muon
  if ((histdir.find("srse") != std::string::npos)) region_label += ", 0 muon, 1 elec";
...
void printTable( vector<TFile*> samples , vector<string> names , vector<string> dirs, string caption = "", bool compare = false, int mt2bin = -1 ) {
  ...
  for( unsigned int ix = 0 ; ix < n ; ix++ ){
    if( !TString(names.at(ix)).Contains("sig") ) continue;
    ofile << getTableName(names.at(ix));
    for ( unsigned int idir = 0; idir < ndirs; ++idir ) {
      TString fullhistname = Form("%s/h_mt2bins",dirs.at(idir).c_str());
      TH1D* h = (TH1D*) samples.at(ix)->Get(fullhistname);
      double yield = 0.;
      double err = 0.;
      if (h) {
        // use all bins
...
void plotMaker(){
  ...
  //string input_dir = "/home/users/sicheng/MT2Analysis/MT2looper/output/srsmNew/";
  //string input_dir = "/home/users/sicheng/MT2Analysis/MT2looper/output/noMT2/";
  ...
  //string input_dir = "/home/users/sicheng/play/MT2Analysis/MT2looper/output/test/";
  //string input_dir = "/home/users/sicheng/MT2Analysis/MT2looper/output/noiso/";
  //string input_dir = "/home/users/sicheng/MT2Analysis/MT2looper/output/rawBabyOld/";

  //string subdir = "absiso_mt2g200/";
  ...
  // TFile* f_T5WW4q_305_300 = new TFile(Form("%s/T5qqqqWW_305_300.root",input_dir.c_str()));
  ...
  if(printplots){
    ...
    //TIter it(f_T5WW4q_315_300->GetListOfKeys());
    TKey* k;
