...
for m1 in range(0, x_max+1, x_binwidth):
    for m2 in range(0, y_max+1, y_binwidth):
        ...
        cmd = "nice -n 19 python makeSignalPoint.py {0} {1} {2} {3} &> {4}".format(TAG, MODEL, m1, m2, logname)
        points.append((m1,m2))
        cmds.append(cmd)
...
while done < len(points):
    ...
    for cmd in cmds[current:current+MAXLOCALJOBS-running]:
        ...
        current += 1
    if done < len(points):
        print "{0}/{1} points done. {2} currently running. Checking again in 5 seconds...".format(done, len(points), running)
        time.sleep(5)
