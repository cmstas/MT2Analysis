...
void plot_trigeff_Photon200 (const TString& indir = "/nfs-6/userdata/mt2/V00-09-02_json_294927-302663_PromptReco_18p26fb") {
  ...
  TCut had = base + "nJet30 > 0 && gamma_ht > 1200. && (HLT_PFHT1050 || HLT_PFJet500)";
  //  TCut had = base + "ht > 900";
  // TCut muon = base + " && nMuons10 > 0 && abs(lep_pdgId[0]) == 13 && lep_pt[0] > 25.";

  t_jetht->Draw("gamma_pt[0]>>h_pt_denom_jetht",had);
  t_jetht->Draw("gamma_pt[0]>>h_pt_num_jetht",had+"HLT_Photon200");
  t_jetht->Draw("gamma_pt[0]>>h_pt_eb_denom_jetht",had+eb);
  t_jetht->Draw("gamma_pt[0]>>h_pt_eb_num_jetht",had+eb+"HLT_Photon200");
  t_jetht->Draw("gamma_pt[0]>>h_pt_ee_denom_jetht",had+ee);
  t_jetht->Draw("gamma_pt[0]>>h_pt_ee_num_jetht",had+ee+"HLT_Photon200");

  TH2F* h_axis = new TH2F("h_axis",";#gamma p_{T} [GeV];Efficiency of HLT_Photon200",45,100,1000,20,0,1);
  ...
  leg->AddEntry(h_pt_eff_jetht,"PFHT1050 || PFJet500","pe");
  leg->Draw("same");
  ...
  leg_eb->SetHeader("Ref: PFHT1050 || PFJet500");
