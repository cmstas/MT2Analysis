...
#rootdir="root://redirector.t2.ucsd.edu//store/user/dpgilber"
rootdir="/hadoop/cms/store/user/dpgilber"
...
f_16 = ROOT.TFile.Open(rootdir+"/ShortTrack/output_merged/data_2016_{0}.root".format(tag)) if not useMC else ROOT.TFile.Open(rootdir+"/ShortTrack/output_merged/mc_2016_{0}.root".format(tag))
f_1718 = ROOT.TFile.Open(rootdir+"/ShortTrack/output_merged/data_2017and2018_{0}.root".format(tag)) if not useMC else ROOT.TFile.Open(rootdir+"/ShortTrack/output_merged/mc_2017and2018_{0}.root".format(tag))
if not doScan:
    f_16_contam = ROOT.TFile.Open(rootdir+"/ShortTrack/output_merged/data_2016_{0}_contam.root".format(tag)) if not useMC else ROOT.TFile.Open(rootdir+"/ShortTrack/output_merged/mc_2016_{0}_contam.root".format(tag))
    f_1718_contam = ROOT.TFile.Open(rootdir+"/ShortTrack/output_merged/data_2017and2018_{0}_contam.root".format(tag)) if not useMC else ROOT.TFile.Open(rootdir+"/ShortTrack/output_merged/mc_2017and2018_{0}_contam.root".format(tag))
#f_sig_16 = ROOT.TFile.Open(rootdir+"/ShortTrack/output_unmerged/2016_{0}/signal/{1}.root".format(tag,signal))
#f_sig_16_GENMET = ROOT.TFile.Open(rootdir+"/ShortTrack/output_unmerged/2016_{0}/signal/{1}_GENMET.root".format(tag,signal))
    f_sig_1718 = ROOT.TFile.Open(rootdir+"/ShortTrack/output_unmerged/2017and2018_{0}/signal/{1}.root".format(tag,signal))
    f_sig_16 = ROOT.TFile.Open(rootdir+"/ShortTrack/output_unmerged/2016_{0}/signal/{1}.root".format(tag,signal))
    f_sig_1718_GENMET = ROOT.TFile.Open(rootdir+"/ShortTrack/output_unmerged/2017and2018_{0}/signal/{1}_GENMET.root".format(tag,signal))
    f_sig_16_GENMET = ROOT.TFile.Open(rootdir+"/ShortTrack/output_unmerged/2016_{0}/signal/{1}_GENMET.root".format(tag,signal))
else:
    f_16_sigscan = ROOT.TFile.Open(rootdir+"/ShortTrackScan/output_merged/{}_{}_2016.root".format(signal,tag))
    if f_16_sigscan.IsZombie():
        print "2016 signal not found"
        exit(1)
    f_1718_sigscan = ROOT.TFile.Open(rootdir+"/ShortTrackScan/output_merged/{}_{}_2017and2018.root".format(signal,tag))
    if f_1718_sigscan.IsZombie():
        print "2017-18 signal not found"
        exit(1)
    f_16_contam = ROOT.TFile.Open(rootdir+"/ShortTrackScan/output_merged/data_2016_{}_{}_contam.root".format(signal,tag))
    if f_16_contam.IsZombie():
        print "2016 contam not found"
        exit(1)
    f_1718_contam = ROOT.TFile.Open(rootdir+"/ShortTrackScan/output_merged/data_2017and2018_{}_{}_contam.root".format(signal,tag))
    if f_1718_contam.IsZombie():
        print "2017-18 contam not found"
        exit(1)
...
def makeTemplate(year,region,length):
    ...
    h_fs = f.Get("h_FS_"+("1" if region.find("1") >= 0 else ("23" if region.find("23") >= 0 else "4"))+ptstring).Clone("h_fs_{}".format(channel))
    good_fshort = True
    ...
    if fs <= 0:
        good_fshort = False
        if length > 1:
            print "fs = {:.3f} in {}, using fshort of next lowest track length".format(fs,channel)
        else:
            print "fs = {:.3f} in {} for P tracks, aborting".format(fs,channel)
            exit(1)
        fs = h_fs.GetBinContent(length-1)
        fserr_stat_up = 1+(h_fs_stat_up.GetBinError(length-1)/fs)
        fserr_stat_dn = 1-(h_fs_stat_dn.GetBinError(length-1)/fs)
        if fserr_stat_dn < 0: fserr_stat_dn = 0.001
        # fully correlate the error with the track length category we're using
        fshort_id_string = "{}_{}{}_{}".format(["P","P3","P4","M","L"][length-2],"23" if region.find("23") > 0 else ("1" if region[0] == "1" > 0 else "4"),ptstring,year)
    else:
    ...
    elif False: # code for fake monojet
...
def makeCardScan(year,region,template,signal,outdir,length,n_bkg,im1=-1,im2=-1):
    ...
    f_sigscan = f_16_sigscan if year == "2016" else f_1718_sigscan

    sigscan_name = "h_{}_{}{}_ST".format(njht,MT2,pt)
#    sigscan_reconame = "h_{}_{}{}_ST_RECOMET".format(njht,MT2,pt)
    sigscan_delta_name = "h_{}_{}{}_ST_RECO_MINUS_GENMET".format(njht,MT2,pt)
    sigscan_isr_name = "h_{}_{}{}_ST_DELTA_ISR".format(njht,MT2,pt)
    h_sigscan = f_sigscan.Get(sigscan_name).Clone("{}_{}_{}".format(channel,im1,im2))
#    h_recoscan = f_sigscan.Get(sigscan_reconame).Clone("{}_{}_{}_RECO".format(channel,im1,im2))
...
#        rescale_sig = 1 if not rescale17 else 35.9/41.5 * rescale_2016
    ...
    if year == "2017and2018": 
        f_contam = f_1718_contam
        rescale_sig = 1 #(41.37 + 59.66) / 41.529

    delta_recogen = h_deltascan.GetBinContent(length,biny,binz) * rescale_sig # = gen - reco met counts
#    n_sig_reco = h_recoscan.GetBinContent(length,biny,binz) * rescale_sig # = pure reco met counts
    n_sig = h_sigscan.GetBinContent(length,biny,binz) * rescale_sig # GEN and RECO MET averaged counts
#    print channel,n_sig
    mcstat_sig = 1 + h_sigscan.GetBinError(length,biny,binz) * rescale_sig / n_sig if n_sig > 0 else 1 

#    isr_sig = 1.10
    # Set ISR and RECO/GEN MET errors to half the fractional difference between rescaled counts and unrescaled counts
    delta_isr = h_isrscan.GetBinContent(length,biny,binz) * rescale_sig
    isr_sig = 1.0 + abs(delta_isr)/2/n_sig if n_sig > 0 else 1.0

    # error assessed due to difference between reco and gen sig results
    genmet_sig = 1.0 + abs(delta_recogen)/2/n_sig if n_sig > 0 else 1.0
    ...
    if subtractSignalContam:
        ...
        if n_sig > 0:
            ...
            try:
                ...
                print "No contam hist for",region
                exit(1)
    ...
    if n_sig == 0 and suppressZeroBins: return 0
...
#f_weights_16 = ROOT.TFile.Open("{}/2016/nsig_weights_{}.root".format(rootdir+"/babymaker/data/short_track",signal))
#h_nevents_16_temp = f_weights_16.Get("h_nsig")
#h_nevents_16 = h_nevents_16_temp.Clone("h_nsig_16")
#h_nevents_16.SetDirectory(0)
#f_weights_16.Close()                             
#f_weights_17 = ROOT.TFile.Open("{}/2017/nsig_weights_{}.root".format(rootdir+"/babymaker/data/short_track",signal))
...
#                    if True:
                        m1 = int(round(im1))
                        im2 = 0
#                        nsig_x = h_nevents_16.GetXaxis().FindBin(m1)
                        while im2 < im1:
#                        im2 = 1525
#                        if True:                            
                            m2 = int(round(im2))
...
#print ""
#for mass_point in rescaled_points:
#    print mass_point
