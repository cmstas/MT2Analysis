...
void printDetailedTableDataDriven(vector<TFile*> samples, vector<string> names, vector<string> dirs, string caption = "") {
  ...
  for (unsigned int idir = 0; idir < ndirs; ++idir) {
    ...
    for (unsigned int i = 0; i < n; i++) { // need to find a sample that has this hist filled
      h_mt2bins = (TH1D*) samples.at(i)->Get(binshistname);
      if (h_mt2bins) {
  ...
  for (int i = 0; i < ndirs; ++i)
    for (int j = 0; j < nmt2bins[i]; ++j) { ofile << "|c"; ++ncols; }
  ofile << "}" << std::endl;
  ofile << "\\hline\\hline" << endl;
  ofile << "\\multirow{2}{*}{Sample}";
  for (unsigned int idir = 0; idir < ndirs; ++idir)
  ...
  for (unsigned int i = 0; i < ndirs; ++i)
    for (unsigned int j = 0; j < nmt2bins[i]; ++j)
      ofile << " & " << "  ";   // FIXME
...
TH1F* makeSRYieldsPlot(vector<TFile*> samples, vector<string> dirs, string suffix = "") {
  const unsigned int nsamp = samples.size();
  const unsigned int ndirs = dirs.size();

  int n_srbins = 18;            // for now
...
void plotMakerHcand() {
  ...
  if (printplots) {
    ...
    while ((k = (TKey *)it())) {
      ...
      // // makePlot( samples , names , dir_name , "h_mt2"+s , "M_{T2} [GeV]" , "Events / 50 GeV" , 100 , 1000 , 5 , false, printplots, scalesig, doRatio, scaleBGtoData );
      ...
      // makePlot( samples , names , dir_name , "h_nJet30"+s , "N(jets)" , "Events" , 0 , 15 , 1 , false, printplots, scalesig, doRatio, scaleBGtoData );
