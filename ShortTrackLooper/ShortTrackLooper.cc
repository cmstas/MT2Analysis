...
void ShortTrackLooper::loop(TChain* chain, std::string sample, std::string output_dir){
  ...
  if ((doScanWeights || applyBtagSF) && ((sample.find("T1") != std::string::npos) || (sample.find("T2") != std::string::npos) || (sample.find("T5") != std::string::npos) || (sample.find("T6") != std::string::npos))) {
  ...
  while ( (currentFile = (TFile*)fileIter.Next()) ) {
	...
	cout << "WARNING: bad event with unphysical jet pt! " << t.run << ":" << t.lumi << ":" << t.evt
      ...
      // flag signal samples
      if (t.evt_id >= 1000 || sample.find("T5") != std::string::npos) isSignal_ = true;
